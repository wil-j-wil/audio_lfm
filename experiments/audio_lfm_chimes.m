...
len = [350 350]; % range of demodulation lengthscales (different frequencies tend to need different lengthscales)
...
analysisRange = 2001:16000; % analysis range (chosen manually - smallest representative chunk of the signal)
DS = ceil(min(demod_struct.len)/5); % downsample rate
...
lfmOpt.fbinit = 0.075; % initial guess for feedback terms
...
RemoveNoiseFloor = 1; % set to >0 to remove noise floor
playSounds = 0;
