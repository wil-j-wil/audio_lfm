...
fileName = 'stim117_dog_drinking'; % name of wav file
...
analysisRange = 27000:35000; % analysis range (chosen manually - smallest representative chunk of the signal)
DS = 20; % downsample rate
N = 5; % # envelopes to include in first (main) LFM (all other envelopes are appended to the model optimised on the N envelopes with the greatest energy)
...
lfmOpt.sig_noise = 0.004; % assumed measurement noise variance (automatically calculated below)
...
% FEEDBACK
lfmOpt.fb_terms = 9; % number of additional feedback terms
lfmOpt.sparsity_fb = [0 0 1 0 0 1 0 0 1]; % index which feedback terms to include (omit to optimise all)
...
lfmOpt.sparsity_fwd = [0 1 0 0 0 1]; % index which feedback terms to include (omit to optimise all)
