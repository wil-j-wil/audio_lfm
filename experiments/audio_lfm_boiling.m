...
fileName = 'stim35_boiling_water'; % name of wav file
...
analysisRange = 23201:30000; % analysis range (chosen manually - smallest representative chunk of the signal)
DS = ceil(1.16*min(demod_struct.len)/5); % downsample rate
...
lfmOpt.dt = max(DS*100/demod_struct.fs,0.1); %0.1; % time step size
...
% FEEDBACK
lfmOpt.fb_terms = 5; % number of additional feedback terms
lfmOpt.sparsity_fb = [0 0 0 0 1]; % index which feedback terms to include (omit to optimise all)
...
% FORWARD
lfmOpt.fwd_terms = 5; % number of additional forward terms
lfmOpt.sparsity_fwd = [0 0 0 0 1]; % index which feedback terms to include (omit to optimise all)
