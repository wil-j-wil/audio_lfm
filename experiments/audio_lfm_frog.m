...
fileName = 'stim162_frog_croaking'; % name of wav file
...
analysisRange = 1810:13000; % analysis range (chosen manually - smallest representative chunk of the signal)
DS = 15; % downsample rate
...
lfmOpt.silenceThreshold = 0.025; % if maxmimum amplitude drops below this value times the max of the signal, don't make predictions
...
% FEEDBACK
lfmOpt.fb_terms = 6; % number of additional feedback terms
lfmOpt.sparsity_fb = [0 0 1 0 0 1]; % index which feedback terms to include (omit to optimise all)
...
% FORWARD
lfmOpt.fwd_terms = 6; % number of additional forward terms
lfmOpt.sparsity_fwd = [0 0 1 0 0 1]; % index which feedback terms to include (omit to optimise all)
