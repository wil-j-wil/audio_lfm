...
fileName = 'rain'; % name of wav file
...
len = [150 150]; % range of demodulation lengthscales (different frequencies tend to need different lengthscales)
...
analysisRange = 401:5000; % analysis range (chosen manually - smallest representative chunk of the signal)
DS = 20; % downsample rate
N = 8; % # envelopes to include in first (main) LFM (all other envelopes are appended to the model optimised on the N envelopes with the greatest energy)

lfmOpt = struct; % store the LFM settings in a structure
lfmOpt.analysisRange = analysisRange;
lfmOpt.DS = DS;
lfmOpt.nlf = 4; % # latent forces
...
lfmOpt.sig_noise = 0.0015; % assumed measurement noise variance (automatically calculated below)
...
% FEEDBACK
lfmOpt.fb_terms = 4; % number of additional feedback terms
lfmOpt.sparsity_fb = [0 0 0 1]; % index which feedback terms to include (omit to optimise all)
...
% FORWARD
lfmOpt.fwd_terms = 4; % number of additional forward terms
lfmOpt.sparsity_fwd = [0 0 0 1]; % index which feedback terms to include (omit to optimise all)
