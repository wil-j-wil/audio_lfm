...
len = [250]; % range of demodulation lengthscales (different frequencies tend to need different lengthscales)
...
analysisRange = 20201:29100; % analysis range (chosen manually - smallest representative chunk of the signal)
...
lfmOpt.sig_noise = 0.001; % assumed measurement noise variance (automatically calculated below)
lfmOpt.silenceThreshold = 0; % if maxmimum amplitude drops below this value times the max of the signal, don't make predictions
...
maxEn = max(envs(:,analysisRange),[],2) / max(max(envs(:,analysisRange),[],2));
totEn = sum(envs(:,analysisRange),2) / max(sum(envs(:,analysisRange),2));
En = maxEn + totEn;
[~,I] = sort(En,'descend');
lfmData.Energy = En;
lfmData.I = I;
...
if playSounds == 1
    ...
    pause(length(lfmData.demod_struct.y)/lfmData.demod_struct.fs + 0.5)
    sound(nmfSig,lfmData.demod_struct.fs)
end
