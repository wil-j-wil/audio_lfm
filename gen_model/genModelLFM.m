function [cascadeMod, newLF] = genModelLFM(f_ind, sig_dur, suffix)

% function to generate novel sounds using the LFM-based model
...
lengthscales = {   4,      5,    5,   20,   10,   20,   13,...
                16.6,     20,   50,    5,   10,   30,   11,...
                  20,      4,   25,    5,   75,  100, 11.6,...
                  30,   31.2,   50, 16.6,   50};
            
fA_prctile   = {70, 20, 60, 30, 70, 20, 20,...
                20, 20, 20, 20, 20, 20, 20,...
                20, 40, 00, 40, 20, 40, 20,...
                20, 40, 00, 60, 20};
            
scale_sparse = {  [3 2],   [0.5 10],    [2 2],     [1 7],  [1.2 4],  [0.95 5], [0.95 5],...
                 [1 50],  [0.95 10],    [1 5], [0.95 10],    [2 4], [0.95 10],  [1.6 4],...
                [1.6 4],  [1.2 3.5], [4.75 3],   [2 2.5],  [1 100],  [0.95 5],  [2 3],...
                [1.2 5],    [1 100],  [1 100],     [2 6], [0.95 5]};

%f_ind = 8; % file name index, e.g. 8 = applause
fileName = fileNames{f_ind};

fprintf('generating LFM sample for %s\n',fileName);
...
%%
force = lfmData.filterFinal.uAdjusted;
forceVar = lfmData.filterFinal.uVar;
force_pos = log(1+exp(force));
force_pos(find(force_pos<=0.0001)) = 0.0001; %#ok<FNDSB>
force_posVar = log(1+exp(forceVar));
...
forceC = bsxfun(@rdivide,force_pos,fA');
...
subplot(511)
plot(force_pos')
title('The Latent Forces')
subplot(512)
plot(force_max','b')
hold on
plot(fA,'r')
title('Calculating the High-Level Modulator')
subplot(513)
plot(fA_tr)
title('Transformed High-Level Modulator')
subplot(514)
plot(forceC_adjust')
title('Demodulated & Transformed Latent Forces')
subplot(515)
plot(forceC')
title('Demodulated Latent Forces')
drawnow();
...
for fnum = 1:nlf
    ...
    % Perform inference
      ...
      % The GP covariance function
      gpcf = gpcf_sexp('lengthScale', 1/sqrt(lfmData.filterFinal.theta_opt(fnum)), ...
                           'magnSigma2', 1.5, ...
                           'lengthScale_prior', pl, ...
                           'magnSigma2_prior', pm);
...
for suf = 1:length(suffix)
    ...
    sparseParam = 1*min(max(eGP - eU,-0.2),2)
    ...
    %end
    %fsamp_tr = g_f(fsamp + cascadeMod.mu - sparseParam);
    if max(fsamp_tr)>1.3*max(fA)
        fsamp_tr = (1.3*max(fA)/max(fsamp_tr))*fsamp_tr;
    elseif max(fsamp_tr)<0.8*max(fA)
        fsamp_tr = (0.8*max(fA)/max(fsamp_tr))*fsamp_tr;
    end
    ...
    for fnum=1:nlf
        ...
        sparseParam = -abs(newLF{fnum}.mu)/10;
        warning('off','all');
        ...
        if max(fsamp_tr)>1.3*max(forceC(fnum,:))
            fsamp_tr = (1.3*max(forceC(fnum,:))/max(fsamp_tr))*fsamp_tr;
        elseif max(fsamp_tr)<0.9*max(forceC(fnum,:))
            fsamp_tr = (0.9*max(forceC(fnum,:))/max(fsamp_tr))*fsamp_tr;
        end
    ...
    if saveSound == 1
        audiowrite(strcat('../audio/synth/',fileName,'_LFM',suffix{suf},'.wav'),sigSynth,lfmData.demod_struct.fs);
    end
