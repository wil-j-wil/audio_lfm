function [cascadeMod, newLF] = genModelNMF(f_ind, sig_dur, suffix)

% function to generate novel sounds using the NMF-based model
...
fA_prctile   = {70, 20, 60, 30, 60, 20, 20,...
                20, 20, 20, 20, 20, 20, 20,...
                20, 20, 00, 20, 20, 40, 20,...
                20, 40, 00, 60, 20};
            
scale_sparse = {   [3 2],   [0.5 10],    [2 2],     [1 7],   [1.2 4],      [2 5],    [2 4],...
                [0.8 50],  [0.95 10],    [1 5], [0.95 10],     [5 2],  [0.95 10], [1.5 10],...
                   [3 2],     [1 10], [4.75 3],   [2 2.5],   [1 100], [0.95 100],    [3 3],...
                 [1.2 5],    [1 100],  [1 100], [0.95 10], [1.25 10]};

%f_ind = 18; % file name index, e.g. 8 = applause
fileName = fileNames{f_ind};

fprintf('generating NMF sample for %s\n',fileName);
...
%fmu = mean(force,2);
%force_adjust = bsxfun(@minus,force,fmu);

force_pos = GPPAD(nmf_h',2,0);
...
forceC = bsxfun(@rdivide,nmf_h,fA');
...
for suf = 1:length(suffix)
    ...
    elseif max(fsamp_tr)<0.9*max(fA)
        fsamp_tr = (0.9*max(fA)/max(fsamp_tr))*fsamp_tr;
    end
    ...
    if saveSound == 1
        audiowrite(strcat('../audio/synth/',fileName,'_NMF',suffix{suf},'.wav'),sigSynth,lfmData.demod_struct.fs);
    end
